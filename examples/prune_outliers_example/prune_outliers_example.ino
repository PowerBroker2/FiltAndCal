...
void setup()
{
  ...
  printMatXd(good_data);
  Serial.println();

  Vector3d bad_data_point;
  bad_data_point << -2, -2, -2;
  MatrixXd noised_data = good_data;
  ...
  printVecXd(bad_data_point);
  Serial.println();
  Serial.println("noised_data");
  printMatXd(noised_data);
  Serial.println();

  VectorXd mean              =  good_data.rowwise().mean();
  MatrixXd good_data_no_mean = (good_data.colwise() - mean);
  Serial.println("mean");
  printVecXd(mean);
  Serial.println();
  
  MatrixXd covar = cov(good_data);
  ...
  MatrixXd pruned_data = prune_gaussian_outliers(noised_data, 95);
  Serial.println("pruned_data");
  printMatXd(pruned_data);
  Serial.println();
}
